...
def _prepare_excel(file_storage):
    ...
    if not telemetry.empty:
        prep(telemetry, ["timestamp", "application"], "Application Telemetry")
    ...
    if not performance.empty:
        prep(performance, ["timestamp", "application"], "Performance Metrics")
    ...
    if problems.empty:
        ...
        _rename(problems, "title", ["problem_title", "name"]); _rename(problems, "severity", ["problem_severity"]); _rename(problems, "status", ["problem_status"])
        _rename(problems, "start_time", ["starttime", "start"]); _rename(problems, "duration_min", ["duration", "duration_minutes"])
...
def mock_problems(start,end,applications):
    ...
    for i,dt in enumerate(dates):
        for j in range(1+int(i%4==0)+int(i%6==0)):
            rows.append({"title":titles[(i+j)%6],"severity":sev[(i+j)%6],"status":"CLOSED","startTime":dt+pd.Timedelta(hours=int(rng.integers(0,20))),"duration_min":int(rng.integers(20,500)),"application":applications[(i+j)%len(applications)]})
...
def _live_data(cfg,management_zone,start,end):
    selector='type("SERVICE")' + (f',mzName("{management_zone}")' if management_zone else '')
    fr=pd.Timestamp(start).strftime("%Y-%m-%dT%H:%M:%SZ"); to=(pd.Timestamp(end)+pd.Timedelta(days=1)-pd.Timedelta(seconds=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
    frames={}
    for key,ms in {"request_count":'builtin:service.requestCount:sum:splitBy("dt.entity.service")',"response_time_ms":'builtin:service.response.time:avg:splitBy("dt.entity.service")'}.items():
        frames[key]=_series(_dt_get(cfg,"/api/v2/metrics/query",{"metricSelector":ms,"entitySelector":selector,"from":fr,"to":to,"resolution":"1h"}))
    ...
    for key,ms in {"cpu_pct":"builtin:host.cpu.usage:avg","memory_pct":"builtin:host.mem.usage:avg","disk_pct":"builtin:host.disk.usedPct:avg"}.items():
        ...
        if f.empty: perf[key]=np.nan
            ...
            f.timestamp=f.timestamp.dt.floor("D"); f=f.groupby("timestamp",as_index=False).value.mean().rename(columns={"value":key}); perf=perf.merge(f,on="timestamp",how="left")
...
def _window(dataset,start,end,application):
    ...
    data=data[(data.timestamp>=s)&(data.timestamp<=e)].copy()
...
def _app_table(data,days):
    ...
    for name,g in data.groupby("application"):
        req=g.request_count.dropna(); resp=g.response_time_ms.dropna(); rs=float(np.polyfit(np.arange(len(req)),req.to_numpy(),1)[0]) if len(req)>2 else 0; ps=float(np.polyfit(np.arange(len(resp)),resp.to_numpy(),1)[0]) if len(resp)>2 else 0; rf=max(0,float(req.iloc[-1]+rs*days)) if len(req) else 0; pf=max(0,float(resp.iloc[-1]+ps*days)) if len(resp) else 0; risk="At Risk" if pf>=500 else ("Watch" if pf>=250 else "Normal"); rows.append({"application":str(name),"requests_avg":float(req.mean()) if len(req) else 0,"response_avg":float(resp.mean()) if len(resp) else 0,"request_forecast":rf,"response_forecast":pf,"status":risk})
...
def build_analysis(dataset,start,end,months,growth,application):
    ...
    days=max(1,int(months)*30); metrics={}; trends={}; forecasts={}
    for key,(label,cat,unit,col) in METRICS.items():
        df=data[["timestamp",col]].rename(columns={col:"value"}).dropna() if col in data else pd.DataFrame(columns=["timestamp","value"]); df=df.groupby("timestamp",as_index=False).mean() if not df.empty else df; f=_forecast(df,days); trends[key]=_trend(df); forecasts[key]=f; metrics[key]={"label":label,"category":cat,"unit":unit,"cap":100 if key in ("host_cpu_usage","host_mem_usage","host_disk_used_pct") else None,"historical":_points(df,"value"),"forecast":_points(f,"forecast"),"lower":_points(f,"lower"),"upper":_points(f,"upper")}
    sf=1+float(growth)/100
    for key,m in metrics.items():
        el=1.15 if key=="service_response_time" else 1; m["simulated_forecast"]=[{"t":p["t"],"v":min(m["cap"],p["v"]*sf**el) if m["cap"] else p["v"]*sf**el} for p in m["forecast"]]
    peaks={k:(float(metrics[k]["forecast"][-1]["v"]) if metrics[k]["forecast"] else 0) for k in ["host_cpu_usage","host_mem_usage","host_disk_used_pct"]}; scenarios=[{"growth":g,"cpu":min(100,peaks["host_cpu_usage"]*(1+g/100)),"memory":min(100,peaks["host_mem_usage"]*(1+g/100)),"disk":min(100,peaks["host_disk_used_pct"]*(1+g/100))} for g in [-20,0,10,25,50]]
    apps=_app_table(data,days)
    return {"management_zone":dataset["management_zone"],"application":application,"summary":{"from":str(pd.Timestamp(start).date()),"to":str(pd.Timestamp(end).date()),"applications":1,"rows":int(len(data)),"problems":int(len(problems)),"baseline_requests":float(data.request_count.mean()) if "request_count" in data else None,"forecast_days":days,"data_source":dataset["kind"]},"metrics":metrics,"application_table":apps,"trends":trends,"forecasts":forecasts,"problems":problems,"scenarios":scenarios,"recommendations":[{"application":r["application"],"risk":r["status"],"action":"Review request growth and response-time headroom before the next planning cycle."} for r in apps if r["status"]!="Normal"],"growth":float(growth)}
...
def get_analysis(source_id,zone,start,end,months,growth,application):
    ...
    if src["kind"] in ("excel","mock"): dataset=src["dataset"]
...
def applications():
    ...
    try:
        if src["kind"] in ("mock","excel"): apps=src["dataset"]["applications"]
